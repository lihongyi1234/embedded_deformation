...
void writePLY(const std::string &filepath, Eigen::MatrixXd &V,
  ...
  tinyply:
    ...
    if (V.cols() != 0) {
      ...
      object_to_write.add_properties_to_element(
    ...
    if (N.cols() == V.cols()) {
      for (int i = 0; i < N.cols(); i++)
        cube.normals.push_back(
            {float(N(0, i)), float(N(1, i)), float(N(2, i))});
    ...
    if (RGB.cols() == V.cols()) {
      for (int i = 0; i < RGB.cols(); i++)
        cube.rgb.push_back(
            {uint8_t(RGB(0, i)), uint8_t(RGB(1, i)), uint8_t(RGB(2, i))});

      object_to_write.add_properties_to_element(
          "vertex", {"red", "green", "blue"}, Type::UINT8, cube.rgb.size(),
          reinterpret_cast<uint8_t *>(cube.rgb.data()), Type::INVALID, 0);
    }
    ...
    if (F.cols() != 0) {
      for (int i = 0; i < F.cols(); i++)
        cube.triangles.push_back(
            {uint32_t(F(0, i)), uint32_t(F(1, i)), uint32_t(F(2, i))});
...
void writePLY(const std::string &filepath, Eigen::MatrixXd &V,
              Eigen::MatrixXi &F, Eigen::MatrixXd &N, bool write_in_ascii) {
  Eigen::MatrixXi RGB;
  writePLY(filepath, V, F, N, RGB, write_in_ascii);
}

void writePLY(const std::string &filepath, Eigen::MatrixXd &V,
              Eigen::MatrixXi &F, bool write_in_ascii) {
  Eigen::MatrixXd N;
  Eigen::MatrixXi RGB;
  writePLY(filepath, V, F, N, RGB, write_in_ascii);
}
...
void writePLY(const std::string &filepath, Eigen::MatrixXd &V,
              Eigen::MatrixXd &N, bool write_in_ascii) {
  Eigen::MatrixXi F;
  Eigen::MatrixXi RGB;
  writePLY(filepath, V, F, N, RGB, write_in_ascii);
}
