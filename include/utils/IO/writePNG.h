...
inline bool writePNG(
  ...
  stbi_write_png(png_file.c_str(), R.cols(), R.rows(), comp, data.data(),
                 stride_in_bytes);

  return true;
}

inline bool writePNG(const Eigen::MatrixXd& R, const Eigen::MatrixXd& G,
...
inline bool writePNG(const Eigen::MatrixXd& R, const Eigen::MatrixXd& G,
                     const Eigen::MatrixXd& B, const std::string png_file) {
  Eigen::MatrixXd A = Eigen::MatrixXd::Ones(R.rows(), R.cols());

  return writePNG(R, G, B, A, png_file);
};

inline bool writePNG(const Eigen::MatrixXd& gray_map,
                     const std::string png_file) {
...
inline bool writePNG(
  ...
  Eigen::MatrixXd A =
      Eigen::MatrixXd::Ones(binary_map.rows(), binary_map.cols());
