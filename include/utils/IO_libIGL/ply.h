...
* was changed to always open files in binary mode, files written in ascii can
also be read with this binary mode.
...
* was changed to handle line breaks in UNIX, MACINTOSH, PC, it resets the file
pointer accordingly for the next read.
...
operating system dependent, as a line break in unix is indicated with the escape
character \n, on a macintosh, with \r, and on a pc with \r\n  <--2 unsigned
chars, 2 bytes, instead of 1 byte.

get_words allows reading of any OS, text editors such as BBEdit do not save the
linebreaks properly to target OSs with binary files.
...
inline void ply_describe_element(PlyFile *, const char *, int, int,
                                 PlyProperty *);
...
inline PlyFile *ply_open_for_reading(const char *, int *, char ***, int *,
                                     float *);
inline PlyProperty **ply_get_element_description(PlyFile *, const char *, int *,
                                                 int *);
...
inline PlyFile *ply_write(FILE *fp, int nelems, const char **elem_names,
                          int file_type) {
  int i;
  PlyFile *plyfile;
  PlyElement *elem;
  ...
  int types_checked = 0;
  if (native_binary_type == -1) native_binary_type = get_native_binary_type2();
  if (!types_checked) types_checked = check_types();
...
inline PlyFile *ply_open_for_writing(const char *filename, int nelems,
                                     const char **elem_names, int file_type,
                                     float *version) {
  PlyFile *plyfile;
  char *name;
  FILE *fp;
...
inline void ply_describe_element(PlyFile *plyfile, const char *elem_name,
...
inline void ply_describe_property(PlyFile *plyfile, const char *elem_name,
                                  PlyProperty *prop) {
  PlyElement *elem;
...
inline void ply_describe_other_properties(PlyFile *plyfile, PlyOtherProp *other,
                                          int offset) {
  int i;
  PlyElement *elem;
  PlyProperty *prop;
...
inline void ply_element_count(PlyFile *plyfile, const char *elem_name,
                              int nelems) {
  PlyElement *elem;
  ...
  if (plyfile->file_type == PLY_ASCII) {
    ...
    for (j = 0; j < elem->nprops; j++) {
      ...
      if (prop->is_list) {
        ...
        get_stored_item((void *)item, prop->count_internal, &int_val, &uint_val,
                        &double_val);
        write_binary_item(fp, plyfile->file_type, int_val, uint_val, double_val,
                          prop->count_external, native_binary_type);
        list_count = uint_val;
        item_ptr = (char **)(elem_data + prop->offset);
        item = item_ptr[0];
        item_size = ply_type_size[prop->internal_type];
        ...
        get_stored_item((void *)item, prop->internal_type, &int_val, &uint_val,
                        &double_val);
        write_binary_item(fp, plyfile->file_type, int_val, uint_val, double_val,
                          prop->external_type, native_binary_type);
      }
    }
  }
}
...
inline void ply_put_comment(PlyFile *plyfile, char *comment) {
  ...
  if (plyfile->num_comments == 0)
    ...
    plyfile->comments = (char **)realloc(
        plyfile->comments, sizeof(char *) * (plyfile->num_comments + 1));
...
inline void ply_put_obj_info(PlyFile *plyfile, char *obj_info) {
  ...
  if (plyfile->num_obj_info == 0)
    ...
    plyfile->obj_info = (char **)realloc(
        plyfile->obj_info, sizeof(char *) * (plyfile->num_obj_info + 1));
...
inline PlyFile *ply_open_for_reading(char *filename, int *nelems,
...
inline PlyProperty **ply_get_element_description(PlyFile *plyfile,
...
inline void ply_get_element_setup(PlyFile *plyfile, const char *elem_name,
...
inline void ply_get_property(PlyFile *plyfile, const char *elem_name,
                             PlyProperty *prop) {
  PlyElement *elem;
...
inline PlyOtherProp *ply_get_other_properties(PlyFile *plyfile,
                                              const char *elem_name,
                                              int offset) {
  int i;
  PlyElement *elem;
...
inline PlyOtherElems *ply_get_other_element(PlyFile *plyfile, char *elem_name,
                                            int elem_count) {
  int i;
  PlyElement *elem;
...
inline void ply_describe_other_elements(PlyFile *plyfile,
                                        PlyOtherElems *other_elems) {
  int i;
  OtherElem *other;
  PlyElement *elem;
  ...
  REALLOCN(plyfile->elems, PlyElement *, plyfile->nelems,
           plyfile->nelems + other_elems->num_elems);
...
inline PlyProperty *find_property(PlyElement *elem, const char *prop_name,
                                  int *index) {
  int i;
...
inline void binary_get_element(PlyFile *plyfile, char *elem_ptr,
  ...
  for (j = 0; j < elem->nprops; j++) {
    ...
    if (prop->is_list) { /* a list */
      ...
      /* get and store the number of items in the list */
      get_binary_item(fp, plyfile->file_type, prop->count_external, &int_val,
                      &uint_val, &double_val, native_binary_type);
      if (store_it) {
        item = elem_data + prop->count_offset;
        store_item(item, prop->count_internal, int_val, uint_val, double_val);
      }
      ...
      if (list_count == 0) {
        ...
        // read items and store them into the array
        for (k = 0; k < list_count; k++) {
          get_binary_item(fp, plyfile->file_type, prop->external_type, &int_val,
                          &uint_val, &double_val, native_binary_type);
          if (store_it) {
            store_item(item, prop->internal_type, int_val, uint_val,
                       double_val);
...
#define BIG_STRING 4096
  ...
  for (ptr = str, ptr2 = str_copy; *ptr != '\0'; ptr++, ptr2++) {
    *ptr2 = *ptr;
    ...
    if (*ptr == '\t') {
      ...
      *ptr = ' ';  // has to have a space, to be caught later when grouping
                   // words
      *ptr2 = '\0';
      break;
    } else if (*ptr == '\r') {  // MAC line break
      nonUNIX = 1;
  ...
  *nwords = num_words;
  *orig_line = str_copy;  // ToDo: This looks like UB, returns pointer to local
                          // variable on stack.
  return (words);
}
...
inline void write_binary_item(FILE *fp, int file_type, int int_val,
                              unsigned int uint_val, double double_val,
                              int type, int *native_binary_type) {
...
inline void write_ascii_item(FILE *fp, int int_val, unsigned int uint_val,
...
inline void get_stored_item(void *ptr, int type, int *int_val,
                            unsigned int *uint_val, double *double_val) {
  switch (type) {
    case PLY_CHAR:
      *int_val = *((char *)ptr);
...
inline void get_binary_item(FILE *fp, int file_type, int type, int *int_val,
                            unsigned int *uint_val, double *double_val,
                            int *native_binary_type) {
  char c[8];
  void *ptr;
...
inline void get_ascii_item(char *word, int type, int *int_val,
                           unsigned int *uint_val, double *double_val) {
  switch (type) {
    case PLY_CHAR:
    case PLY_UCHAR:
...
inline void store_item(char *item, int type, int int_val, unsigned int uint_val,
                       double double_val) {
  unsigned char *puchar;
  short int *pshort;
  unsigned short int *pushort;
...
inline void add_element(PlyFile *plyfile, char **words) {
  ...
  if (plyfile->nelems == 0)
    ...
    plyfile->elems = (PlyElement **)realloc(
        plyfile->elems, sizeof(PlyElement *) * (plyfile->nelems + 1));
...
inline int get_prop_type(char *type_name) {
  ...
  for (i = PLY_START_TYPE + 1; i < PLY_END_TYPE; i++)
    if (equal_strings(type_name, alt_type_names[i])) return (i);
