...
///   5) Two main input types are Eigen::Matrix3Xd or
...
/// without duplicating the data storage. This code is adapted from the
/// KDTreeEigenMatrixAdaptor class of nanoflann.hpp
...
struct KDTreeAdaptor {
  ...
  index_t* index;
  KDTreeAdaptor(const MatrixType& mat, const int leaf_max_size = 10)
      : m_data_matrix(mat) {
...
Eigen::Affine3d point_to_point(Eigen::MatrixBase<Derived1>& X,
  ...
  if (svd.matrixU().determinant() * svd.matrixV().determinant() < 0.0) {
    ...
    S(2) = -1.0;
    transformation.linear().noalias() =
        svd.matrixV() * S.asDiagonal() * svd.matrixU().transpose();
  } else {
    transformation.linear().noalias() =
        svd.matrixV() * svd.matrixU().transpose();
  }
  transformation.translation().noalias() =
      Y_mean - transformation.linear() * X_mean;
...
#pragma omp section
      for (int i = 0; i < C.cols(); i++) {
        double dist_to_plane =
            -((X.col(i) - Y.col(i)).dot(N.col(i)) - u(i)) * w(i);
...
namespace SICP {
struct Parameters {
  bool use_penalty = false;  /// if use_penalty then penalty method else ADMM or
                             /// ALM (see max_inner)
  double p = 1.0;            /// p norm
...
inline double shrinkage(double mu, double n, double p, double s) {
  return shrinkage<I - 1>(
      mu, n, p, 1.0 - (p / mu) * std::pow(n, p - 2.0) * std::pow(s, p - 1.0));
}
template <>
...
void point_to_point(Eigen::MatrixBase<Derived1>& X,
  ...
  /// Build kd-tree
  nanoflann::KDTreeAdaptor<Eigen::MatrixBase<Derived2>, 3,
...
#pragma omp parallel for
    ...
    for (int outer = 0; outer < par.max_outer; ++outer) {
      ...
      for (int inner = 0; inner < par.max_inner; ++inner) {
        /// Z update (shrinkage)
        Z = (Qn.array() * (X - Qp).array()).colwise().sum().transpose() +
            C.array() / mu;
        shrink<3>(Z, mu, par.p);
        /// Rotation and translation update
        Eigen::VectorXd U = Z - C / mu;
        RigidMotionEstimator::point_to_plane(
            X, Qp, Qn, Eigen::VectorXd::Ones(X.cols()), U);
        /// Stopping criteria
        dual = (X - Xo1).colwise().norm().maxCoeff();
        Xo1 = X;
        if (dual < par.stop) break;
      ...
      /// C update (lagrange multipliers)
      Eigen::VectorXf P =
          (Qn.array() * (X - Qp).array()).colwise().sum().transpose() -
          Z.array();
      if (!par.use_penalty) C.noalias() += mu * P;
      /// mu update (penalty)
      if (mu < par.max_mu) mu *= par.alpha;
...
namespace ICP {
enum Function { PNORM, TUKEY, FAIR, LOGISTIC, TRIMMED, NONE };
class Parameters {
 public:
  Parameters() : f(NONE), p(0.1), max_icp(100), max_outer(100), stop(1e-5) {}
  /// Parameters
...
void tukey_weight(Eigen::VectorXd& r, double p) {
  for (int i = 0; i < r.rows(); ++i) {
    if (r(i) > p)
      ...
      r(i) = std::pow((1.0 - std::pow(r(i) / p, 2.0)), 2.0);
  }
}
/// @param Residuals
/// @param Parameter
...
/// @param Target (one 3D point per column)
/// @param Parameters
void point_to_point(Eigen::Matrix3Xd& X, Eigen::Matrix3Xd& Y,
                    Parameters par = Parameters()) {
  /// Build kd-tree
  nanoflann::KDTreeAdaptor<Eigen::Matrix3Xd, 3, nanoflann::metric_L2_Simple>
      kdtree(Y);
  /// Buffers
  Eigen::Matrix3Xd Q = Eigen::Matrix3Xd::Zero(3, X.cols());
