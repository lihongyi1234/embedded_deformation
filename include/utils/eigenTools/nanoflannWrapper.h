...
class nanoflann_wrapper {
 ...
 public:
  nanoflann_wrapper(Eigen::MatrixXd& target) {
    ...
    this->kd_tree_index =
        std::make_shared<nanoflann::KDTreeEigenMatrixAdaptor<Eigen::MatrixXd> >(
            dimensionality, this->pointcloud_, leaf_size);
  ...
  bool return_k_closest_points(Eigen::Vector3d query_point, int k,
                               std::vector<int>& indexes,
  ...
  std::vector<int> radius_search(Eigen::Vector3d query_point, double max_dist) {
    ...
    this->kd_tree_index->index->radiusSearch(&query_pt.at(0), max_dist, matches,
                                             params);
  ...
  bool radius_search(Eigen::Vector3d query_point, double max_dist,
                     std::vector<int>& indexes,
                     std::vector<double>& distances) {
    // Query point:
    std::vector<double> query_pt;
