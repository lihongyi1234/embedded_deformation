...
inline bool plot_two_meshes(const Eigen::MatrixXd& source_V,
  ...
  polyscope::registerSurfaceMesh("first mesh", source_V.transpose(),
                                 source_F.transpose());
  polyscope::getSurfaceMesh("first mesh")
      ->setSurfaceColor(glm::vec3{0.1, 0.1, 1});

  polyscope::registerSurfaceMesh("second mesh", target_V.transpose(),
                                 target_F.transpose());
  polyscope::getSurfaceMesh("second mesh")
      ->setSurfaceColor(glm::vec3{1, 0.1, 0.1});
