										   ...
										   options opts)
{
	...
	// build edges:
	printf("out cloud size %d\n", int(N.rows()));
	Eigen::MatrixXi E(N.rows() * (int(nodes_connectivity_) + 1), 2);
	nanoflann_wrapper tree_2(N);
	int counter = 0;
	for (int i = 0; i < N.rows(); ++i) {
		std::vector< int > closest_points;
...
void EmbeddedDeformation::deform(Eigen::MatrixXd sources_in, Eigen::MatrixXd targets_in, Eigen::MatrixXd & V_deformed)
{
	...
	for (int i = 0; i < deformation_graph_ptr_->num_nodes(); ++i)
	{
		...
		problem.AddResidualBlock(new ceres::NumericDiffCostFunction<RotCostFunction_2_V2, ceres::CENTRAL, 9, 12>(
			new RotCostFunction_2_V2(w_rot_)
			),
	...
	for (int i = 0; i < deformation_graph_ptr_->num_nodes(); ++i)
	{
		for (int j = 0; j < deformation_graph_ptr_->get_adjacency_list(i).size(); ++j)
		{
			...
			problem.AddResidualBlock(new ceres::NumericDiffCostFunction<RegCostFunction_V2, ceres::CENTRAL, 3, 12, 12>(
				new RegCostFunction_V2(w_reg_, g_j, g_k)
				),
				new ceres::HuberLoss(1.0),
				&params[i][0], &params[k][0]
			);
		}
	}
	...
	for (int i = 0; i < sources.rows(); ++i)
	{
		...
		problem.AddResidualBlock(new ceres::NumericDiffCostFunction<ConCostFunction_V2, ceres::CENTRAL, 3, 12,12,12,12,12,12>(
			new ConCostFunction_V2(w_con_, vector_g, sources.row(i), targets.row(i))
			),
			new ceres::HuberLoss(1.0),
			//parameter_blocks
			&params[sources_nodes_neighbours[i][0]][0],
			&params[sources_nodes_neighbours[i][1]][0],
			&params[sources_nodes_neighbours[i][2]][0],
			&params[sources_nodes_neighbours[i][3]][0],
			&params[sources_nodes_neighbours[i][4]][0],
			&params[sources_nodes_neighbours[i][5]][0]
		);
...
void EmbeddedDeformation::deform_V2(Eigen::MatrixXd sources_in, Eigen::MatrixXd targets_in, Eigen::MatrixXd& V_deformed)
{
	{
		Eigen::MatrixXd sources, targets;
		...
		for (int i = 0; i < deformation_graph_ptr_->num_nodes(); ++i)
		{
			for (int j = 0; j < deformation_graph_ptr_->get_adjacency_list(i).size(); ++j)
			{
				...
				problem.AddResidualBlock(new ceres::NumericDiffCostFunction<RegCostFunction_V3, ceres::CENTRAL, 3, 7, 7>(
					new RegCostFunction_V3(w_reg_, g_j, g_k)
					),
					new ceres::HuberLoss(1.0),
					&params[i][0], &params[k][0]
				);
		...
		for (int i = 0; i < sources.rows(); ++i)
		{
			...
			problem.AddResidualBlock(new ceres::NumericDiffCostFunction<ConCostFunction_V3, ceres::CENTRAL, 3, 7, 7, 7, 7, 7, 7>(
				new ConCostFunction_V3(w_con_, vector_g, sources.row(i), targets.row(i))
				),
				new ceres::HuberLoss(1.0),
				//parameter_blocks
				&params[sources_nodes_neighbours[i][0]][0],
		...
		for (int i = 0; i < deformation_graph_ptr_->num_nodes(); ++i)
		{
			Eigen::Map<const Eigen::Vector3d> wxyz_i(&(params[i][0]));
			Eigen::Quaterniond q_i(wxyz_i[0], wxyz_i[1], wxyz_i[2], wxyz_i[3]);
			q_i = q_i.normalized();
			rotation_matrices_[i] = q_i.toRotationMatrix();
			translations_[i] = Eigen::Map<Eigen::Vector3d>(&(params[i][0]) + 4);
		}
